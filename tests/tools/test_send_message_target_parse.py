...
def test_registered_plugin_target_is_explicit_native_id() -> None:
    ...
    try:
        chat_id, thread_id, is_explicit = _parse_target_ref("fakeim", "dm-alpha")
    finally:
        platform_registry.unregister("fakeim")

    assert chat_id == "dm-alpha"
    assert thread_id is None
    assert is_explicit is True
...
def test_send_message_fails_closed_when_gateway_runner_lookup_crashes() -> None:
    whatsapp_cfg = SimpleNamespace(enabled=True, token=None, extra={"api_url": "http://bridge"})
    config = SimpleNamespace(
        platforms={Platform.WHATSAPP: whatsapp_cfg},
        get_home_channel=lambda _platform: None,
    ...
    with patch("gateway.config.load_gateway_config", return_value=config), \
         patch("tools.interrupt.is_interrupted", return_value=False), \
         patch("gateway.run._gateway_runner_ref", side_effect=RuntimeError("runner unavailable")), \
         patch("model_tools._run_async", side_effect=_run_async_immediately), \
         patch("tools.send_message_tool._send_to_platform", new=AsyncMock()) as send_mock:
        result = json.loads(
            send_message_tool(
...
def test_send_message_uses_explicit_cli_hook_registry() -> None:
    whatsapp_cfg = SimpleNamespace(enabled=True, token=None, extra={"api_url": "http://bridge"})
    config = SimpleNamespace(
        platforms={Platform.WHATSAPP: whatsapp_cfg},
        get_home_channel=lambda _platform: None,
...
def test_send_message_routes_registered_plugin_native_id_without_home_fallback() -> None:
    platform_registry.register(
        PlatformEntry(
            name="fakeim",
            label="Fake IM",
    ...
    try:
        ...
        with patch("gateway.config.load_gateway_config", return_value=config), \
             patch("tools.interrupt.is_interrupted", return_value=False), \
             patch("gateway.channel_directory.resolve_channel_name", side_effect=AssertionError("plugin native id should not resolve via directory")), \
             patch("gateway.run._gateway_runner_ref", return_value=runner), \
             patch("model_tools._run_async", side_effect=_run_async_immediately), \
             patch("tools.send_message_tool._send_to_platform", new=AsyncMock(return_value={"success": True})) as send_mock, \
             patch("gateway.mirror.mirror_to_session", return_value=True):
